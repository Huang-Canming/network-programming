...
int readFromSocket(int iSocketFd, char *pcBuf, int iBufSize){
	int iMsgLed = 0;
	
	while (iMsgLed <= iBufSize)
    {
        iMsgLed = recv(iSocketFd, pcBuf, BufSize, 0);             // 通用的读文件设备的接口，默认阻塞
        if (iMsgLed < 0) {
            printf("Server failed to read from network, err[%s]\n", strerror(errno));
            return iMsgLed;
        }
        else if (0 == iNetMsgLen) {                              `// 因为 recv 阻塞，所以不会走到这里，可以设置超时时间
...
void* socketServer(void* param){
	...
	iNetMsgLen = readFromSocket(iConnFd, szNetMsg, sizeof(szNetMsg));
	if (iNetMsgLen < 0) {
		printf("Server failed to read from network, err[%s]\n", strerror(errno));
		close(iConnFd);
		close(iLsnFd);
